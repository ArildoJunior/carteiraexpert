...
const tests = [
  ...
  check('Finnhub (AAPL)', 'FINNHUB_TOKEN', async (t) => {
    if (!t) return { ok: false, detail: 'chave vazia' };
    const r = await fetchWithTimeout(
      `https://finnhub.io/api/v1/quote?symbol=AAPL&token=${t}`
    );
    if (r.status === 401 || r.status === 403) {
  ...
  check('Twelve Data (AAPL)', 'TWELVE_DATA_TOKEN', async (t) => {
    if (!t) return { ok: false, detail: 'chave vazia' };
    const r = await fetchWithTimeout(
      `https://api.twelvedata.com/price?symbol=AAPL&apikey=${t}`
    );
    if (r.status === 401) return { ok: false, detail: 'chave inválida (401)' };
    if (!r.ok) return { ok: false, detail: `HTTP ${r.status}` };
    const j = await r.json();
  ...
  check('Financial Modeling Prep (income-stmt bulk AAPL)', 'FINANCIAL_MODELING_PREP_TOKEN', async (t) => {
    if (!t) return { ok: false, detail: 'chave vazia' };
    ...
    return { ok: true, detail: `revenue=$${(j?.[0]?.revenue / 1e9).toFixed(1)}B, ${j?.length} periods` };
  }),
  ...
  check('CoinCap (BTC)', 'COINCAP_TOKEN', async () => {
    ...
    if (!r.ok) return { ok: false, detail: `HTTP ${r.status}` };
    const j = await r.json();
    return { ok: true, detail: `$${parseFloat(j?.data?.priceUsd).toFixed(0)}` };
  }),
