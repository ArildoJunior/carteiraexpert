...
// ─── Serviços de Eventos Corporativos (Re-exportados de corporate-actions/server) ─
export {
  createCorporateActionEventInTransaction,
  createCorporateActionEvent,
  createBonusEventInTransaction,
  createBonusEvent,
  createIncomeEventInTransaction,
  createIncomeEvent,
} from '@/modules/corporate-actions/server/corporate-action.service';
...
export async function listUserRecentEvents(
  ...
  return rows.map(({ event, portfolioName, assetTicker, assetName, assetMarket }) => ({
    ...
    direction: event.direction as PortfolioEvent['direction'],
