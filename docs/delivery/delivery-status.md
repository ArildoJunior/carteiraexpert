...
- [x] **Testes Unitários:** Comprovados no repositório (**93 arquivos e 1.115 testes aprovados** via `pnpm run test:unit`, cobrindo motores determinísticos de projeção, valuation teórico, schemas, gráficos, preferências, catálogo, billing, quotas, planos, contas de caixa e custódia).
...
- [x] **Testes End-to-End (E2E):** Suítes Playwright estruturadas (**13 arquivos e 180 testes aprovados** via `pnpm run test:e2e` cobrindo simulador de juros compostos `/simulador`, contas de custódia, importações completas com resolução explícita de ativos e isolamento IDOR, autenticação, consentimento LGPD, carteiras, catálogo público desktop/mobile, quotas de planos, subscrições e preferências de gráficos em Chromium 60/60, Firefox 60/60 e WebKit 60/60).
