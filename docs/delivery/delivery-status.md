...
- **Fase 06 — Dados de Mercado, Valuation e Gráficos:** **PACOTES 06.01 E 06.02 HOMOLOGADOS COM SUCESSO (`PASS`); PACOTE 06.03 PLANEJADO (ADR-010)** (Infraestrutura interna entregue: tabelas `market_quotes`, `exchange_rates` e `user_chart_preferences` via migrações `0005` e `0010`, adaptadores `ManualPayloadAdapter`, `MockProviderAdapter` e conector externo público `BrapiAdapter`, script CLI `scripts/ingest-market-data.ts`, serviço de ingestão `MarketDataIngestionService` com ranking de qualidade, motores determinísticos de valuation e evolução temporal diária "Mercado vs. Custo", gráficos Recharts de alocação por ativo/classe/moeda e evolução temporal com seletores interativos, persistência atômica de preferências visuais por usuário e área no PostgreSQL com fila serializada anti-concorrência `ChartPreferenceSyncQueue`, coalescência, proteção de estado local contra `router.refresh()` e isolamento multitenant estrito. O **Pacote 06.03** encontra-se especificado e planejado conforme [ADR-010](../decisions/ADR-010-b3-eod-historical-ingestion.md) e [`phase-06-03-b3-historical-market-data-ingestion.md`](./phase-06-03-b3-historical-market-data-ingestion.md) para ingestão histórica e atualização diária B3 COTAHIST via upload privado e processamento assíncrono).
...
- **Ingestão Histórica e Atualização Diária B3 (Pacote 06.03 / ADR-010):** Upload privado de arquivos ZIP de séries históricas e fim de dia (COTAHIST), parser de largura fixa, validação estrutural de header/trailer, carga idempotente em `market_quotes` e processamento assíncrono por workers em background;
