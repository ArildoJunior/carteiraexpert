...
- **Etapa 9 — Módulo Fiscal Dedicado e Relatórios Auxiliares de IRPF:** **IMPLEMENTADA E VALIDADA** (Módulo `src/modules/tax/`, migração `0022_add_tax_calculation_tables.sql`; tabelas `tax_calculation_runs`, `tax_monthly_summaries` e `tax_loss_credits`; motor puramente determinístico em `Decimal` em `tax-engine.ts`; apuração mensal de ganho/perda de capital líquido com preço médio ponderado contínuo e ajuste em eventos corporativos; regra de isenção de R$ 20.000,00 em ações no mercado à vista conforme IN RFB 2054/2024 com trava estrita de não compensação de prejuízos em meses isentos; tributação de Day-Trade a 20% sem isenção de 20k; apuração segregada de FIIs com rendimentos isentos na fonte; JCP com separação de IRRF 15% retido na fonte e dividendos isentos; compensação FIFO de prejuízos acumulados válidos por 5 anos; interface visual `/fiscal` com banner de avisos regulatórios CVM/RFB, modal de parametrização de alíquotas/isenção, cards mensais detalhados por ativo, abas completas de auxílio à declaração anual — Bens e Direitos em 31/12 com discriminação textual, Rendimentos Isentos, Tributação Exclusiva, Controle de Prejuízos — e exportação em CSV e impressão/PDF; sem emissão de DARF e sem integração oficial com a Receita Federal).
...
23. `user_chart_preferences`: Preferências de visualização de gráficos por usuário e área (`portfolio_evolution`, `dashboard_allocation`, `portfolio_allocation`, `tax_preferences`);
