...
| **06** | **Dados de Mercado, Valuation e Gráficos** | Fases 03–04 | **Implementada e homologada** | Banco local (`market_quotes`, `exchange_rates`, `user_chart_preferences`), adaptadores, valuation, evolução e preferências persistidas por usuário/área. |
...
| **07** | **Importações Revisáveis** | Fases 03–04 | **Pronta para homologação (fluxos comprovados)** | Módulo de importação CSV (`carteiraexpert_csv`, `b3_trades_csv`, `b3_movements_csv`), limite de 5 MB, deduplicação SHA-256, tela de revisão, edição com `Decimal`, resolução explícita de ativos e confirmação transacional atômica em `portfolio_events`. |
