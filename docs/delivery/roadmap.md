...
O alinhamento executivo para testes reais organiza as entregas em 10 etapas sequenciais. O catálogo físico do banco de dados relacional é composto exatamente por **40 tabelas físicas de aplicação** (além da tabela de controle `__drizzle_migrations`, totalizando 41 tabelas no PostgreSQL), validadas pelo Schema Guardian.
