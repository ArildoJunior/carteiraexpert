...
- **Próxima Fase Prevista:** **Fase 05 — Planos, Entitlements e Compartilhamento**.
...
### 8. Sistema Global de Tema e Identidade Visual
...
### 9. Integridade de Schema, Contratos e Banco de Dados
- **Schema Guardian:** Validação física em tempo de execução (`assertSchemaCompatible`) e via CLI (`db:verify`) inspecionando o catálogo PostgreSQL (14 tabelas oficiais validadas).
...
├── tests/
│   ├── unit/                    # Testes unitários puros (motores, schemas, tema, adaptadores)
│   ├── integration/             # Testes de integração com PostgreSQL real (carteiras, market data, subscrições)
│   └── types/                   # Fixtures de tipagem estática (database-contracts.test-d.ts)
├── e2e/                         # Testes end-to-end com Playwright (autenticação, termos, subscrições)
...
pnpm db:verify        # Inspecionar catálogo físico no banco principal (14 tabelas)
...
1. **Fase 05 — Planos Comerciais e Entitlements:** Catálogo comercial de assinaturas, limites de carteiras por plano (2 no Free, 10 em planos superiores), gateways de pagamento e gestão de grupos compartilhados (ADR-004).
