...
export class StooqAdapter implements ProviderAdapter<GlobalQuoteInput, GlobalQuoteOutput> {
  readonly name = "stooq";
  ...
  async fetch(input: GlobalQuoteInput): Promise<GlobalQuoteOutput> {
    ...
    const ticker = rawTicker.includes(".")
      ? rawTicker.toLowerCase()
      : `${rawTicker.toLowerCase()}.US`;
    ...
    return {
      ...
      marketTime:
        date && time ? new Date(`${date}T${time}Z`).toISOString() : new Date().toISOString(),
      source: "stooq",
    };
  }
}
