...
export class CoinGeckoAdapter implements ProviderAdapter<CryptoQuoteInput, CryptoQuoteOutput> {
  ...
  async ping(): Promise<boolean> {
    ...
    try {
      const r = await fetch(`${COINGECKO_BASE}/ping?x_cg_demo_api_key=${this.token}`, {
  ...
  async fetch(input: CryptoQuoteInput): Promise<CryptoQuoteOutput> {
    ...
    const coin = json?.[0];
    if (!coin) {
      throw new ProviderDataError(this.name, this.category, `Cripto "${symbol}" não encontrada`);
    }
