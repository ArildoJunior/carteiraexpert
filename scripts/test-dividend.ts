...
async function fetchBR(registry: Registry, symbol: string): Promise<void> {
  ...
  const result = await registry.fetch<BrDividendInput, BrDividendOutput>(
    'dividend_br',
    { ticker: symbol },
  );
  ...
  result.data.dividends.slice(0, 10).forEach((d) => {
...
async function fetchUS(registry: Registry, symbol: string): Promise<void> {
  const start = Date.now();
  const result = await registry.fetch<UsDividendInput, UsDividendOutput>(
    'dividend_us',
    { ticker: symbol },
  );
  const total = Date.now() - start;
